-- Secure server-side invite proof table
...
-- Helpful uniqueness for pending invites per tenant+email
CREATE UNIQUE INDEX IF NOT EXISTS ux_user_invites_tenant_email_pending
ON public.user_invites (tenant_id, lower(email))
WHERE consumed_at IS NULL;

-- Harden trigger: never trust plain invited flag, require invite_code proof from server record
...
BEGIN
  ...
  ELSE
    -- Staff invite claim: require cryptographic invite_code created server-side
    BEGIN
...
-- Lock down vulnerable RPC from direct client use
